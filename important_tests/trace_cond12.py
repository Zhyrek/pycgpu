#!/usr/bin/env python
"""Trace single AlCuFe condition (T=700, X_AL=0.6, X_CU=0.1): CPU debug vs GPU verbose.
...
conds = {v.X('AL'): 0.6, v.X('CU'): 0.1, v.T: 700, v.P: 101325, v.N: 1}
...
for mode, use_gpu in [('CPU', False), ('GPU', True)]:
    ...
    for j in range(len(ph)):
        if isinstance(ph[j], str) and ph[j]:
            print(f"  phase {ph[j]:12s} NP={npv[j]:.9f}")
