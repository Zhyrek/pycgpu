...
__device__ PhaseRecord g_phase_records_array[3]; // Must be at least 1
...
__global__ void init_all_gpu_phase_records() {
    ...
    #endif
        g_phase_records_array[0].init(&pycgpu_model_0_obj, &pycgpu_model_0_formulaobj, &pycgpu_model_0_formulagrad, &pycgpu_model_0_formulahess, &pycgpu_model_0_internal_cons_func, &pycgpu_model_0_internal_cons_jac, &pycgpu_model_0_mass_obj, &pycgpu_model_0_formulamole_obj, &pycgpu_model_0_formulamole_grad, 3, 3, 3, 2, 2);
    ...
    g_phase_records_array[2].init(&pycgpu_model_2_obj, &pycgpu_model_2_formulaobj, &pycgpu_model_2_formulagrad, &pycgpu_model_2_formulahess, &pycgpu_model_2_internal_cons_func, &pycgpu_model_2_internal_cons_jac, &pycgpu_model_2_mass_obj, &pycgpu_model_2_formulamole_obj, &pycgpu_model_2_formulamole_grad, 3, 2, 3, 1, 2);
...
__global__ void top_level_equilibrium_kernel(
    ...
    if (tid < 3) {
        #ifdef VERBOSE_DEBUG
        printf("GPU DEBUG: top_level_equilibrium_kernel STARTED with tid=%d, num_conditions=%d, stride=%d\n", tid, num_conditions_total, stride);
        #endif
    }
    
    // BATCH PROCESSING: Each thread may process multiple conditions with stride
    // This allows us to cap memory usage while handling any number of conditions
    for (int condition_idx_batch = tid; condition_idx_batch < num_conditions_total; condition_idx_batch += stride) {
        
        // GLOBAL MEMORY SETUP: Use tid for memory indexing (not condition_idx_batch)
        // This ensures each thread uses its own memory slice regardless of which condition it's processing
        int thread_idx = tid;  // Memory index based on thread ID
    ...
    if (tid < num_conditions_total && results_list_ptr_raw != nullptr) {
        ...
        int condition_idx = condition_idx_batch;  // Use batch index
        
        // Bounds check already handled by for loop condition
        ...
        if (initial_data_byte_array != nullptr && condition_idx < num_conditions_total) {
            ...
            for (int ph_idx = 0; ph_idx < safe_num_phases; ++ph_idx) {
                ...
                // Validate phase data
                bool phase_valid = (phase_amount > 1e-12 && phase_record_idx >= 0 && phase_record_idx < 3);
            ...
            device_phase_data.num_unique_phase_records = 3;
    ...
    } // End of batch processing loop (condition_idx_batch)
}
