...
__device__ double pycgpu_model_0_obj(const double* x) {
    ...
    double x1 = 2.22*x0;
    double x2 = x0*x[3];
    double x3 = (x[3] - x[5])*x2;
    double x4 = 1043.0*x0 + 504.0*x3;
    double x5 = -1.0*x4;
    double x6 = 1e-09 + x5;
    double x7 = 1.41071428571429/x[2];
    double x8 = (1.0/6.0)*pow(x[2], 3);
    double x9 = (1.0/135.0)*pow(x[2], 9);
    double x10 = pow(x6, 15);
    double x11 = pow(x[2], 15);
    double x12 = (1.0/600.0)*x11;
    double x13 = 1e-09 + x4;
    double x14 = pow(x13, 15);
    double x15 = (1.0/10.0)/pow(x[2], 5);
    double x16 = (1.0/315.0)/x11;
    double x17 = (1.0/1500.0)/pow(x[2], 25);
    double x18 = x[3] + x[4] + x[5];
    double x19 = pow(x18, -1);
    double x20 = 8.3145*x[2]*x19;
    double x21 = x[7]*x[4];
    double x22 = x[7]*x[3];
    double x23 = x22*x[4];
    double x24 = (1.0/3.0)*(1 - x18);
    double x25 = x0*x[4];
    double x26 = x25*x[3];
    double x27 = 1.0*x19;
    double x28 = pow(x[2], 3.0);
    double x29 = pow(x[2], -1.0);
    double x30 = x[2]*log(x[2]);
    double x31 = pow(x[2], 2.0);
    double x32 = pow(x[2], -9.0);
    double x33 = 74092.0*x29;
    return x20*(1.0*((1e-15 < x[3]) ? (
   x[3]*log(x[3])
)
: (
   0
...
: (
   ...
   1225.7 + 124.134*x[2] - 5.8927e-08*x28 + 77359.0*x29 - 23.5143*x30 - 0.00439752*x31
)
: ((1811.0 <= x[2]) ? (
   -25383.581 + 299.31255*x[2] - 46.0*x30 + 2.29603e+31*x32
)
: (
   0
))) + x21*(4017.0 - 1.255*x[2] + ((x[2] < 1357.77) ? (
   -7770.458 + 130.485235*x[2] + 1.29223e-07*x28 + 52478.0*x29 - 24.112392*x30 - 0.00265684*x31
)
: ((1357.77 <= x[2]) ? (
...
: (
   0
)))) + x22*(10083.0 - 4.813*x[2] + ((x[2] < 700.0) ? (
   -7976.15 + 137.093038*x[2] - 8.77664e-07*x28 - 24.3671976*x30 - 0.001884662*x31 + x33
)
: (((x[2] < 933.47 && 700.0 <= x[2])) ? (
   -11276.24 + 223.048446*x[2] - 5.764227e-06*x28 - 38.5844296*x30 + 0.018531982*x31 + x33
)
: ((933.47 <= x[2]) ? (
...
: (
   0
)))))) + x27*(3089.2*x3 + 150000.0*x0*x[6] + 4.0*x2*(-30740.0 + 7.9972*x[2]) + 100000.0*x21*x[6] + x23*(-73554.0 + 4.0*x[2]) + x25*(39258.0 - 4.14983*x[2]) - 29249.2599*x26*(x[5] + x24) + x22*x[6]*(160000.0 - 1.0*x[2]) + x26*(-267841.207 + 131.456548*x[2])*(x[3] + x24) + x26*(-103924.225 + 69.9194286*x[2])*(x[4] + x24) + (x[3] - x[4])*x23*(51500.0 - 11.84*x[2])) + x20*log(1 + x1/((x1 <= 0) ? (
   -1.0
)
: (
   1.0
)))*((x[2] < x5) ? (
   1 - 0.641731208021339*(x6*x7 + 1.43058350100604*(x12/x10 + x9/pow(x6, 9) + x8/pow(x6, 3)))
)
: ((x[2] < x4) ? (
   1 - 0.641731208021339*(x7*x13 + 1.43058350100604*(x12/x14 + x8/pow(x13, 3) + x9/pow(x13, 9)))
)
: (((1043.0*x[7]*x[5] + 504.0*(x[3] - x[5])*x[7]*x[3]*x[5] < x[2] && 0 < 1043.0*x[7]*x[5] + 504.0*(x[3] - x[5])*x[7]*x[3]*x[5])) ? (
   -0.641731208021339*(pow(x13, 25)*x17 + x14*x16 + x15*pow(x13, 5))
)
: (((-1.0*(1043.0*x[7]*x[5] + 504.0*(x[3] - x[5])*x[7]*x[3]*x[5]) < x[2] && 1043.0*x[7]*x[5] + 504.0*(x[3] - x[5])*x[7]*x[3]*x[5] < 0)) ? (
   -0.641731208021339*(x10*x16 + pow(x6, 5)*x15 + pow(x6, 25)*x17)
)
: (
   0
)))));
...
__device__ double pycgpu_model_0_formulaobj(const double* x) {
    ...
    double x32 = pow(x[2], -9.0);
    double x33 = 74092.0*x29;
    return 1.0*x18*(x20*(1.0*((1e-15 < x[3]) ? (
   x[3]*log(x[3])
)
: (
   0
...
: (
   0
    ...
    double x20 = x16 + x17 + x18 + x19;
    double x21 = x[2]*x13;
    double x22 = (1.0/3.0)*(1 - x11);
    double x23 = x[3] + x22;
    double x24 = x[7]*x[4];
    double x25 = x24*x[5];
    double x26 = x25*x[3];
...
: (
   1.0
    ...
    double x38 = 1 + x34*x37;
    double x39 = log(x38);
    double x40 = 1043.0*x[5];
    double x41 = x[3] - x[5];
    double x42 = x41*x27;
    double x43 = 504.0*x42;
    double x44 = x40*x[7] + x43*x[3];
    double x45 = -1.0*x44;
    double x46 = 1e-09 + x45;
    double x47 = pow(x[2], -1);
    double x48 = 1.41071428571429*x47;
    double x49 = pow(x46, -3);
    double x50 = pow(x[2], 3);
    double x51 = (1.0/6.0)*x50;
    double x52 = pow(x46, -9);
    double x53 = pow(x[2], 9);
    double x54 = (1.0/135.0)*x53;
    double x55 = pow(x46, 15);
    double x56 = pow(x55, -1);
    double x57 = pow(x[2], 15);
    double x58 = (1.0/600.0)*x57;
    double x59 = x[2] < x45;
    double x60 = 1e-09 + x44;
    double x61 = pow(x60, -3);
    double x62 = pow(x60, -9);
    double x63 = pow(x60, 15);
    double x64 = pow(x63, -1);
    double x65 = x[2] < x44;
    double x66 = pow(x60, 5);
    double x67 = pow(x[2], -5);
    double x68 = (1.0/10.0)*x67;
    double x69 = pow(x57, -1);
    double x70 = (1.0/315.0)*x69;
    double x71 = pow(x60, 25);
    double x72 = pow(x[2], -25);
    double x73 = (1.0/1500.0)*x72;
    double x74 = (1043.0*x[7]*x[5] + 504.0*(x[3] - x[5])*x[7]*x[3]*x[5] < x[2] && 0 < 1043.0*x[7]*x[5] + 504.0*(x[3] - x[5])*x[7]*x[3]*x[5]);
    double x75 = pow(x46, 5);
    double x76 = pow(x46, 25);
    double x77 = (-1.0*(1043.0*x[7]*x[5] + 504.0*(x[3] - x[5])*x[7]*x[3]*x[5]) < x[2] && 1043.0*x[7]*x[5] + 504.0*(x[3] - x[5])*x[7]*x[3]*x[5] < 0);
    double x78 = ((x59 == 1) ? (
   1 - 0.641731208021339*(x46*x48 + 1.43058350100604*(x51*x49 + x54*x52 + x58*x56))
)
: ((x65 == 1) ? (
   1 - 0.641731208021339*(x60*x48 + 1.43058350100604*(x61*x51 + x62*x54 + x64*x58))
)
: ((x74 == 1) ? (
   -0.641731208021339*(x68*x66 + x70*x63 + x71*x73)
)
: ((x77 == 1) ? (
   -0.641731208021339*(x70*x55 + x73*x76 + x75*x68)
)
: (
   0
)))));
    double x79 = x78*x39;
    double x80 = pow(x[2], 1.0);
    double x81 = log(x[2]);
    double x82 = 24.3671976*x81;
    double x83 = pow(x[2], 2.0);
    double x84 = pow(x83, -1);
    double x85 = -74092.0*x84;
    double x86 = x[2] < 700.0;
    double x87 = 38.5844296*x81;
    double x88 = (x[2] < 933.47 && 700.0 <= x[2]);
    double x89 = pow(x[2], -10.0);
    double x90 = 31.748192*x81;
    double x91 = 933.47 <= x[2];
    double x92 = 23.5143*x81;
    double x93 = x[2] < 1811.0;
    double x94 = 46.0*x81;
    double x95 = 1811.0 <= x[2];
    double x96 = 24.112392*x81;
    double x97 = x[2] < 1357.77;
    double x98 = 31.38*x81;
    double x99 = 1357.77 <= x[2];
    double x100 = ((x35 == 1) ? (
   0
)
: (
   0
    ...
    double x101 = x78/x38;
    double x102 = -18.45819*x[2]*x27*x12*x101*x100;
    double x103 = pow(x[2], 2);
    double x104 = 0.905299382744389/x103;
    double x105 = 0.0229512519569*pow(x[2], 14);
    double x106 = 0.0612033385517333*pow(x[2], 8);
    double x107 = 0.459025039138*x103;
    double x108 = (1.0/60.0)/pow(x[2], 26);
    double x109 = (1.0/21.0)/pow(x[2], 16);
    double x110 = (1.0/2.0)/pow(x[2], 6);
    double x111 = x39*x21;
    double x112 = 1.0*x11;
    double x113 = 100000.0*x[7];
    double x114 = x113*x[4];
    double x115 = 3089.2*x42;
    double x116 = 150000.0*x27;
    double x117 = 51500.0 - 11.84*x[2];
    double x118 = x30*x117;
    double x119 = -73554.0 + 4.0*x[2];
    double x120 = x119*x[3];
    double x121 = -103924.225 + 69.9194286*x[2];
    double x122 = x31*x121;
    double x123 = x25*x122;
    double x124 = 29249.2599*(x[5] + x22);
    double x125 = x25*x124;
    double x126 = 4.0*(-30740.0 + 7.9972*x[2]);
    double x127 = 39258.0 - 4.14983*x[2];
    double x128 = x24*x127;
    double x129 = 160000.0 - 1.0*x[2];
    double x130 = x129*x[6];
    double x131 = -267841.207 + 131.456548*x[2];
    double x132 = x23*x131;
    double x133 = x114*x[6] + x115*x[3] + x116*x[6] + x123*x[3] - x125*x[3] + x128*x[5] + x24*x120 + x26*x132 + x28*x126 + x29*x130 + x32*x118;
    double x134 = pow(x[2], 3.0);
    double x135 = pow(x80, -1);
    double x136 = pow(x[2], -9.0);
    double x137 = 4017.0 - 1.255*x[2] + ((x97 == 1) ? (
   -7770.458 + 130.485235*x[2] + 1.29223e-07*x134 + 52478.0*x135 - 0.00265684*x83 - x[2]*x96
)
: ((x99 == 1) ? (
   -13542.026 + 183.803828*x[2] + 3.64167e+29*x136 - x[2]*x98
)
: (
   0
)));
    double x138 = x137*x[7];
    double x139 = 74092.0*x135;
    double x140 = 10083.0 - 4.813*x[2] + ((x86 == 1) ? (
   -7976.15 + 137.093038*x[2] - 8.77664e-07*x134 + x139 - 0.001884662*x83 - x[2]*x82
)
: ((x88 == 1) ? (
   -11276.24 + 223.048446*x[2] - 5.764227e-06*x134 + x139 + 0.018531982*x83 - x[2]*x87
)
: ((x91 == 1) ? (
   -11278.378 + 188.684153*x[2] - 1.230524e+28*x136 - x[2]*x90
)
: (
   0
))));
    double x141 = x140*x[3];
    double x142 = ((x93 == 1) ? (
   1225.7 + 124.134*x[2] - 5.8927e-08*x134 + 77359.0*x135 - 0.00439752*x83 - x[2]*x92
)
: ((x95 == 1) ? (
   -25383.581 + 299.31255*x[2] + 2.29603e+31*x136 - x[2]*x94
)
: (
   0
)));
    double x143 = x142*x[7];
    double x144 = x138*x[4] + x141*x[7] + x143*x[5];
    double x145 = 1.0*(x[2]*x14 + x33*x133 + x33*x144 + x79*x21);
    double x146 = x24*((x97 == 1) ? (
   0
)
: ((x99 == 1) ? (
   0
...
: (
   0
    ...
    double x147 = 9749.7533*x26;
    double x148 = 3089.2*x28;
    double x149 = (2.0/3.0)*x26;
    double x150 = (1.0/3.0)*x26;
    double x151 = -x121*x150;
    double x152 = x32*x117;
    double x153 = 504.0*x28;
    double x154 = -x153;
    double x155 = x154 - x43;
    double x156 = 0.905299382744389*x47;
    double x157 = 0.0229512519569*x57;
    double x158 = x157/pow(x46, 16);
    double x159 = 0.0612033385517333*x53;
    double x160 = x159/pow(x46, 10);
    double x161 = pow(x46, 4);
    double x162 = 0.459025039138*x50;
    double x163 = x162/x161;
    double x164 = x153 + x43;
    double x165 = x157/pow(x60, 16);
    double x166 = x159/pow(x60, 10);
    double x167 = pow(x60, 4);
    double x168 = x162/x167;
    double x169 = (1.0/60.0)*x72;
    double x170 = pow(x60, 24)*x169;
    double x171 = (1.0/21.0)*x69;
    double x172 = pow(x60, 14)*x171;
    double x173 = (1.0/2.0)*x67;
    double x174 = x167*x173;
    double x175 = pow(x46, 24)*x169;
    double x176 = pow(x46, 14)*x171;
    double x177 = x161*x173;
    double x178 = pow(x11, -2);
    double x179 = 1.0*x178;
    double x180 = 8.3145*x[2]*x178;
    double x181 = -x10*x180 - x179*x133 - x179*x144 - x79*x180;
    double x182 = -x131*x150;
    double x183 = x15 + x17 + x18;
    double x184 = x102 + x111*((x59 == 1) ? (
   0
)
: ((x65 == 1) ? (
   0
...
: (
   0
    ...
    double x185 = 150000.0*x[6];
    double x186 = 3089.2*x41;
    double x187 = 2.22*x37;
    double x188 = -x34*x100;
    double x189 = x21*x101;
    double x190 = 1043.0*x[7];
    double x191 = 504.0*x41;
    double x192 = x29*x191;
    double x193 = x153 - x190 - x192;
    double x194 = x154 + x190 + x192;
    double x195 = x15 + x16 + x19;
    double x196 = x[3]*x[5];
    double x197 = x[5]*x[4];
    double x198 = x197*x[3];
    double x199 = x191*x196;
    double x200 = -x199 - x40;
    double x201 = x199 + x40;
    out[0] = x112*(x102 + x14 + x111*((x59 == 1) ? (
   x46*x104 - x49*x107 - x52*x106 - x56*x105
)
: ((x65 == 1) ? (
   x60*x104 - x61*x107 - x62*x106 - x64*x105
)
: ((x74 == 1) ? (
   -0.641731208021339*(-x63*x109 - x66*x110 - x71*x108)
)
: ((x77 == 1) ? (
   -0.641731208021339*(-x55*x109 - x75*x110 - x76*x108)
)
: (
   0
))))) + x33*(x24*(-1.255 + ((x97 == 1) ? (
   106.372843 - 0.00531368*x80 + 3.87669e-07*x83 - 52478.0*x84 - x96
)
: ((x99 == 1) ? (
   152.423828 - 3.277503e+30*x89 - x98
)
: (
   0
)))) + x27*((x93 == 1) ? (
   100.6197 - 0.00879504*x80 - 1.76781e-07*x83 - 77359.0*x84 - x92
)
: ((x95 == 1) ? (
   253.31255 - 2.066427e+32*x89 - x94
)
: (
   0
))) + x29*(-4.813 + ((x86 == 1) ? (
   112.7258404 - 0.003769324*x80 - x82 - 2.632992e-06*x83 + x85
)
: ((x88 == 1) ? (
   184.4640164 + 0.037063964*x80 - 1.7292681e-05*x83 + x85 - x87
)
: ((x91 == 1) ? (
   156.935961 + 1.1074716e+29*x89 - x90
)
: (
   0
)))))) + x33*(-4.14983*x25 + 31.9888*x28 + 4.0*x30 + 131.456548*x23*x26 - 1.0*x29*x[6] - 11.84*x30*x32 + 69.9194286*x31*x26) + x79*x13 + (x15 + x20)*x21);
    out[1] = x145 + x112*(x102 + x181 + x111*((x59 == 1) ? (
   -x155*x156 + x155*x158 + x160*x155 + x163*x155
)
: ((x65 == 1) ? (
   -x164*x156 + x165*x164 + x166*x164 + x168*x164
)
: ((x74 == 1) ? (
   -0.641731208021339*(x164*x170 + x164*x172 + x164*x174)
)
: ((x77 == 1) ? (
   -0.641731208021339*(x175*x155 + x176*x155 + x177*x155)
)
: (
   0
))))) + x21*(x20 + 1.0*((x9 == 1) ? (
...
: (
   0
))) + x33*(x146 + x140*x[7]) + x33*(x115 + x118 + x123 - x125 + x147 + x148 + x151 + x130*x[7] + x131*x149 + x24*x119 + x24*x152 + x25*x132 + x27*x126));
    out[2] = x145 + x112*(x181 + x184 + x21*(x183 + x19 + 1.0*((x7 == 1) ? (
   1 + x6
)
: (
   0
))) + x33*(-x118 + x147 + x182 + x113*x[6] + x121*x149 + x27*x127 + x28*x122 - x28*x124 + x28*x132 + x29*x119 + x29*x152) + (x138 + x146)*x33);
    out[3] = x145 + x112*(x181 + x111*((x59 == 1) ? (
   x160*x193 + x163*x193 - x193*x156 + x193*x158
)
: ((x65 == 1) ? (
   x165*x194 + x166*x194 + x168*x194 - x194*x156
)
: ((x74 == 1) ? (
   -0.641731208021339*(x170*x194 + x172*x194 + x174*x194)
)
: ((x77 == 1) ? (
   -0.641731208021339*(x175*x193 + x176*x193 + x177*x193)
)
: (
   0
))))) + x189*(x188 + x187*x[7]) + x21*(x16 + x183 + 1.0*((x1 == 1) ? (
   1 + x0
)
: (
   0
))) + x33*(x128 - x148 + x151 + x182 - 19499.5066*x26 + x185*x[7] + x29*x126 + x29*x186 + x30*x122 - x30*x124 + x30*x132) + (x143 + x146)*x33);
    out[4] = x112*(x184 + x21*(x18 + x195 + 1.0*((x5 == 1) ? (
   1 + x4
)
: (
   0
))) + x33*x146 + x33*(x114 + x116 + x29*x129));
    out[5] = x112*(x111*((x59 == 1) ? (
   -x200*x156 + x200*x158 + x200*x160 + x200*x163
)
: ((x65 == 1) ? (
   -x201*x156 + x201*x165 + x201*x166 + x201*x168
)
: ((x74 == 1) ? (
   -0.641731208021339*(x201*x170 + x201*x172 + x201*x174)
)
: ((x77 == 1) ? (
   -0.641731208021339*(x200*x175 + x200*x176 + x200*x177)
)
: (
   0
))))) + x189*(x188 + x187*x[5]) + x21*(x17 + x195 + 3.0*((x3 == 1) ? (
   1 + x2
)
: (
   0
))) + x33*(100000.0*x[4]*x[6] + x120*x[4] + x122*x198 - x124*x198 + x126*x196 + x127*x197 + x130*x[3] + x185*x[5] + x186*x196 + x198*x132 + x152*x[3]*x[4]) + (x141 + x146 + x137*x[4] + x142*x[5])*x33);
}

__device__ void pycgpu_model_0_formulahess(double* out, const double* x) {
    double x0 = 1043.0*x[7];
    double x1 = x[3] - x[5];
    double x2 = x[7]*x[3];
    double x3 = 504.0*x2;
    double x4 = x1*x3;
    double x5 = x0*x[5] + x4*x[5];
    double x6 = -1.0*x5;
    double x7 = 1e-09 + x6;
    double x8 = pow(x7, 3);
    double x9 = pow(x8, -1);
    double x10 = 0.918050078276*x[2];
    double x11 = pow(x7, -9);
    double x12 = pow(x[2], 7);
    double x13 = 0.489626708413867*x12;
    double x14 = pow(x7, 15);
    double x15 = pow(x14, -1);
    double x16 = 0.3213175273966*pow(x[2], 13);
    double x17 = pow(x[2], 3);
    double x18 = 1.81059876548878/x17;
    double x19 = x[2] < x6;
    double x20 = 1e-09 + x5;
    double x21 = pow(x20, 3);
    double x22 = pow(x21, -1);
    double x23 = pow(x20, 15);
    double x24 = pow(x23, -1);
    double x25 = pow(x20, -9);
    double x26 = x[2] < x5;
    double x27 = pow(x20, 5);
    double x28 = 3/x12;
    double x29 = (16.0/21.0)/pow(x[2], 17);
    double x30 = pow(x20, 25);
    double x31 = (13.0/30.0)/pow(x[2], 27);
    double x32 = (1043.0*x[7]*x[5] + 504.0*(x[3] - x[5])*x[7]*x[3]*x[5] < x[2] && 0 < 1043.0*x[7]*x[5] + 504.0*(x[3] - x[5])*x[7]*x[3]*x[5]);
    double x33 = pow(x7, 5);
    double x34 = pow(x7, 25);
    double x35 = (-1.0*(1043.0*x[7]*x[5] + 504.0*(x[3] - x[5])*x[7]*x[3]*x[5]) < x[2] && 1043.0*x[7]*x[5] + 504.0*(x[3] - x[5])*x[7]*x[3]*x[5] < 0);
    double x36 = x[7]*x[5];
    double x37 = 2.22*x36;
    double x38 = x37 <= 0;
    double x39 = ((x38 == 1) ? (
...
: (
   1.0
    ...
    double x40 = 2.22/x39;
    double x41 = 1 + x40*x36;
    double x42 = log(x41);
    double x43 = x[3] + x[4] + x[5];
    double x44 = pow(x43, -1);
    double x45 = 8.3145*x44;
    double x46 = x42*x45;
    double x47 = x[2]*x46;
    double x48 = pow(x[2], 2);
    double x49 = pow(x48, -1);
    double x50 = 0.905299382744389*x49;
    double x51 = pow(x[2], 14);
    double x52 = 0.0229512519569*x51;
    double x53 = pow(x[2], 8);
    double x54 = 0.0612033385517333*x53;
    double x55 = 0.459025039138*x48;
    double x56 = pow(x[2], -26);
    double x57 = (1.0/60.0)*x56;
    double x58 = pow(x[2], -16);
    double x59 = (1.0/21.0)*x58;
    double x60 = pow(x[2], -6);
    double x61 = (1.0/2.0)*x60;
    double x62 = ((x19 == 1) ? (
   -x52*x15 - x54*x11 + x7*x50 - x9*x55
)
: ((x26 == 1) ? (
   x50*x20 - x52*x24 - x54*x25 - x55*x22
)
: ((x32 == 1) ? (
   -0.641731208021339*(-x57*x30 - x59*x23 - x61*x27)
)
: ((x35 == 1) ? (
   -0.641731208021339*(-x57*x34 - x59*x14 - x61*x33)
)
: 0))));
    double x63 = 36.91638*x[2];
    double x64 = pow(x41, -1);
    double x65 = ((x38 == 1) ? 0
: 0);
    double x66 = x65/pow(x39, 2);
    double x67 = x44*x36;
    double x68 = x67*x66;
    double x69 = x64*x68;
    double x70 = x63*x69;
    double x71 = pow(x[2], 1.0);
    double x72 = pow(x[2], 3.0);
    double x73 = pow(x72, -1);
    double x74 = pow(x[2], -1);
    double x75 = x[2] < 1357.77;
    double x76 = pow(x[2], -11.0);
    double x77 = 1357.77 <= x[2];
    double x78 = x[7]*x[4];
    double x79 = x[2] < 1811.0;
    double x80 = 1811.0 <= x[2];
    double x81 = 148184.0*x73;
    double x82 = x[2] < 700.0;
    double x83 = (x[2] < 933.47 && 700.0 <= x[2]);
    double x84 = 933.47 <= x[2];
    double x85 = 1.0*x44;
    double x86 = 1.41071428571429*x74;
    double x87 = (1.0/6.0)*x17;
    double x88 = pow(x[2], 9);
    double x89 = (1.0/135.0)*x88;
    double x90 = pow(x[2], 15);
    double x91 = (1.0/600.0)*x90;
    double x92 = pow(x[2], -5);
    double x93 = (1.0/10.0)*x92;
    double x94 = pow(x90, -1);
    double x95 = (1.0/315.0)*x94;
    double x96 = pow(x[2], -25);
    double x97 = (1.0/1500.0)*x96;
    double x98 = ((x19 == 1) ? (
   1 - 0.641731208021339*(x7*x86 + 1.43058350100604*(x89*x11 + x9*x87 + x91*x15))
)
: ((x26 == 1) ? (
   1 - 0.641731208021339*(x86*x20 + 1.43058350100604*(x87*x22 + x89*x25 + x91*x24))
)
: ((x32 == 1) ? (
   -0.641731208021339*(x93*x27 + x95*x23 + x97*x30)
)
: ((x35 == 1) ? (
   -0.641731208021339*(x93*x33 + x95*x14 + x97*x34)
)
: 0))));
    double x99 = x64*x98;
    double x100 = x66*x99;
    double x101 = 36.91638*x100;
    double x102 = x67*x101;
    double x103 = 16.629*x44;
    double x104 = x42*x103;
    double x105 = 1e-15 < x[3];
    double x106 = 1.0*((x105 == 1) ? 0
: 0);
    double x107 = 1e-15 < x[4];
    double x108 = 1.0*((x107 == 1) ? 0
: 0);
    double x109 = 1e-15 < x[5];
    double x110 = 1.0*((x109 == 1) ? 0
: 0);
    double x111 = 1e-15 < x[7];
    double x112 = 3.0*((x111 == 1) ? 0
: 0);
    double x113 = 1e-15 < x[6];
    double x114 = 1.0*((x113 == 1) ? 0
: 0);
    double x115 = x110 + x112 + x114;
    double x116 = x108 + x115;
    double x117 = x106 + x116;
    double x118 = 18.45819*x100;
    double x119 = x[2]*x118;
    double x120 = -x67*x119;
    double x121 = x[2]*x45;
    double x122 = x117*x121;
    double x123 = x120 + x122;
    double x124 = pow(x65, 2);
    double x125 = x98/pow(x41, 2);
    double x126 = x36*x124/pow(x39, 3);
    double x127 = x63*x99*x44*x126;
    double x128 = x127 - 40.9771818*x[2]*x44*x124*x125*pow(x[7], 2)*pow(x[5], 2)/pow(x39, 4);
...
: 0);
    double x137 = 8.3145*x136;
    double x138 = (1.0/3.0)*(1 - x43);
    double x139 = x[3] + x138;
    double x140 = 131.456548*x139;
    double x141 = x2*x[5];
    double x142 = x141*x[4];
    double x143 = 31.9888*x2;
    double x144 = 1.0*x[6];
    double x145 = 4.0*x[4];
    double x146 = 4.14983*x36;
    double x147 = x[4] + x138;
    double x148 = 69.9194286*x147;
    double x149 = x[3] - x[4];
    double x150 = 11.84*x149;
    double x151 = x2*x150;
    double x152 = x140*x142 + x142*x148 + x143*x[5] - x146*x[4] - x151*x[4] - x2*x144 + x2*x145;
    double x153 = log(x[2]);
    double x154 = 24.3671976*x153;
    double x155 = pow(x[2], 2.0);
    double x156 = pow(x155, -1);
    double x157 = -74092.0*x156;
    double x158 = 38.5844296*x153;
    double x159 = pow(x[2], -10.0);
    double x160 = 31.748192*x153;
    double x161 = -4.813 + ((x82 == 1) ? (
   112.7258404 - x154 - 2.632992e-06*x155 + x157 - 0.003769324*x71
)
: ((x83 == 1) ? (
   184.4640164 - 1.7292681e-05*x155 + x157 - x158 + 0.037063964*x71
)
: ((x84 == 1) ? (
   156.935961 + 1.1074716e+29*x159 - x160
)
: 0)));
    double x162 = x161*x[3];
    double x163 = 23.5143*x153;
    double x164 = 46.0*x153;
    double x165 = ((x79 == 1) ? (
   100.6197 - 1.76781e-07*x155 - 77359.0*x156 - x163 - 0.00879504*x71
)
: ((x80 == 1) ? (
   253.31255 - 2.066427e+32*x159 - x164
)
: 0));
    double x166 = x165*x[5];
    double x167 = 24.112392*x153;
    double x168 = 31.38*x153;
    double x169 = -1.255 + ((x75 == 1) ? (
   106.372843 + 3.87669e-07*x155 - 52478.0*x156 - x167 - 0.00531368*x71
)
: ((x77 == 1) ? (
   152.423828 - 3.277503e+30*x159 - x168
)
: 0));
    double x170 = x169*x[7];
    double x171 = x162*x[7] + x166*x[7] + x170*x[4];
    double x172 = x123 + x44*x137 + x62*x47 + x85*x152 + x85*x171 + x98*x46;
    double x173 = 1.0*x172;
    double x174 = pow(x43, -2);
    double x175 = x36*x174;
    double x176 = x119*x175;
    double x177 = 504.0*x36;
    double x178 = x1*x177;
    double x179 = x3*x[5];
    double x180 = -x179;
    double x181 = -x178 + x180;
    double x182 = 0.905299382744389*x181;
    double x183 = pow(x7, -16);
    double x184 = 0.0229512519569*x90;
    double x185 = x183*x184;
    double x186 = pow(x7, -10);
    double x187 = 0.0612033385517333*x88;
    double x188 = x187*x186;
    double x189 = pow(x7, 4);
    double x190 = pow(x189, -1);
    double x191 = 0.459025039138*x17;
    double x192 = x190*x191;
    double x193 = x178 + x179;
    double x194 = 0.905299382744389*x74;
    double x195 = pow(x20, -16);
    double x196 = x184*x195;
    double x197 = pow(x20, -10);
    double x198 = x187*x197;
    double x199 = pow(x20, 4);
    double x200 = pow(x199, -1);
    double x201 = x200*x191;
    double x202 = pow(x20, 24);
    double x203 = (1.0/60.0)*x96;
    double x204 = x202*x203;
    double x205 = (1.0/21.0)*x94;
    double x206 = pow(x20, 14);
    double x207 = x206*x193;
    double x208 = (1.0/2.0)*x92;
    double x209 = x199*x193;
    double x210 = pow(x7, 24);
    double x211 = x210*x203;
    double x212 = pow(x7, 14);
    double x213 = x212*x205;
    double x214 = x208*x189;
    double x215 = ((x19 == 1) ? (
   x181*x185 + x181*x188 + x181*x192 - x74*x182
)
: ((x26 == 1) ? (
   -x193*x194 + x196*x193 + x198*x193 + x201*x193
)
: ((x32 == 1) ? (
   -0.641731208021339*(x204*x193 + x205*x207 + x208*x209)
)
: ((x35 == 1) ? (
   -0.641731208021339*(x211*x181 + x213*x181 + x214*x181)
)
: 0))));
    double x216 = 18.45819*x[2];
    double x217 = x69*x216;
    double x218 = -x215*x217;
    double x219 = x116 + 1.0*((x105 == 1) ? (
   1 + x135
)
: 0);
    double x220 = x45*x219;
    double x221 = x46*x215;
    double x222 = x2*x[4];
    double x223 = 11.84*x222;
    double x224 = x36*x[4];
    double x225 = ((x75 == 1) ? 0
: ((x77 == 1) ? 0
: 0));
    double x226 = x225*x[7];
    double x227 = ((x79 == 1) ? 0
: ((x80 == 1) ? 0
: 0));
    double x228 = x227*x[5];
    double x229 = ((x82 == 1) ? 0
: ((x83 == 1) ? 0
: ((x84 == 1) ? 0
: 0)));
    double x230 = x229*x[7];
    double x231 = x226*x[4] + x228*x[7] + x230*x[3];
    double x232 = 1.377075117414*x48;
    double x233 = x232*x190;
    double x234 = 0.5508300469656*x53;
    double x235 = x234*x186;
    double x236 = 0.3442687793535*x51;
    double x237 = x236*x183;
    double x238 = x234*x197;
    double x239 = x232*x200;
    double x240 = x236*x195;
    double x241 = (5.0/2.0)*x60;
    double x242 = (5.0/7.0)*x58;
    double x243 = (5.0/12.0)*x56;
    double x244 = x241*x189;
    double x245 = x212*x242;
    double x246 = x210*x243;
    double x247 = -x62*x217 - x67*x118;
    double x248 = 1.0*x174;
    double x249 = 8.3145*x174;
    double x250 = x98*x42;
    double x251 = x[2]*x249;
    double x252 = x42*x251;
    double x253 = -x174*x137 - x248*x152 - x248*x171 - x250*x249 - x251*x117 - x62*x252;
    double x254 = x130*(x129 + x176 + x218 + x220 + x221 + x247 + x253 + x47*((x19 == 1) ? (
   x233*x181 + x235*x181 + x237*x181 + x49*x182
)
: ((x26 == 1) ? (
   x238*x193 + x239*x193 + x240*x193 + x50*x193
)
: ((x32 == 1) ? (
   -0.641731208021339*(-x207*x242 - x209*x241 - x202*x243*x193)
)
: ((x35 == 1) ? (
   -0.641731208021339*(-x244*x181 - x245*x181 - x246*x181)
)
: 0)))) + x85*(x231 + x161*x[7]) + x85*(64.3312224666667*x142 - x223 + 31.9888*x36 + 4.0*x78 - x144*x[7] + x224*x140 + x224*x148 - x78*x150));
    double x255 = x106 + x115;
    double x256 = x255 + 1.0*((x107 == 1) ? (
   1 + x134
)
: 0);
    double x257 = x45*x256;
    double x258 = ((x19 == 1) ? 0
: ((x26 == 1) ? 0
: ((x32 == 1) ? 0
: ((x35 == 1) ? 0
: 0))));
    double x259 = x69*x258;
    double x260 = -x216*x259;
    double x261 = x46*x258;
    double x262 = x247 + x260 + x261;
    double x263 = x[2]*x261;
    double x264 = x120 + x263;
    double x265 = x122 + x264;
    double x266 = x176 + x265;
    double x267 = x130*(x128 + x253 + x257 + x262 + x266 + x85*(2.79410306666666*x142 - x146 - x151 + 4.0*x2 + x223 + x140*x141 + x141*x148) + (x170 + x231)*x85);
    double x268 = -x0 + x179 - x4;
    double x269 = 0.905299382744389*x268;
    double x270 = x0 + x180 + x4;
    double x271 = x270*x197;
    double x272 = x202*x270;
    double x273 = x206*x270;
    double x274 = x270*x199;
    double x275 = ((x19 == 1) ? (
   x268*x185 + x268*x188 + x268*x192 - x74*x269
)
: ((x26 == 1) ? (
   x201*x270 - x270*x194 + x270*x196 + x271*x187
)
: ((x32 == 1) ? (
   -0.641731208021339*(x203*x272 + x205*x273 + x208*x274)
)
: ((x35 == 1) ? (
   -0.641731208021339*(x211*x268 + x213*x268 + x214*x268)
)
: 0))));
    double x276 = x46*x275;
    double x277 = x106 + x108 + x112;
    double x278 = x114 + x277;
    double x279 = x278 + 1.0*((x109 == 1) ? (
   1 + x131
)
: 0);
    double x280 = x45*x279;
    double x281 = -x66*x37;
    double x282 = x281 + x40*x[7];
    double x283 = x64*x121;
    double x284 = x282*x283;
    double x285 = 4.14983*x[4];
    double x286 = x99*x45;
    double x287 = x253 + x276 + x280 + x286*x282 + x47*((x19 == 1) ? (
   x233*x268 + x235*x268 + x237*x268 + x49*x269
)
: ((x26 == 1) ? (
   x234*x271 + x239*x270 + x270*x240 + x50*x270
)
: ((x32 == 1) ? (
   -0.641731208021339*(-x272*x243 - x273*x242 - x274*x241)
)
: ((x35 == 1) ? (
   -0.641731208021339*(-x268*x244 - x268*x245 - x268*x246)
)
: 0)))) + x62*x284 + x85*(x231 + x165*x[7]) + x85*(-67.1253255333333*x142 + x143 + x222*x140 + x222*x148 - x285*x[7]);
    double x288 = x44*x[7];
    double x289 = x68*x216*x125;
    double x290 = x176 - x217*x275 + x289*x282;
    double x291 = x127 + x290 - x288*x119;
    double x292 = x123 + x291;
    double x293 = x110 + x277;
    double x294 = x293 + 1.0*((x113 == 1) ? (
   1 + x133
)
: 0);
    double x295 = x45*x294;
    double x296 = x264 + x85*x231;
    double x297 = x128 + x296;
    double x298 = x130*(x122 + x262 + x295 + x297 - x2*x85);
    double x299 = 1043.0*x[5];
    double x300 = x[3]*x[5];
    double x301 = 504.0*x300;
    double x302 = x1*x301;
    double x303 = -x299 - x302;
    double x304 = x299 + x302;
    double x305 = x304*x199;
    double x306 = x206*x304;
    double x307 = x202*x304;
    double x308 = x281 + x40*x[5];
    double x309 = x283*x308;
    double x310 = ((x19 == 1) ? (
   x303*x185 + x303*x188 + x303*x192 - x303*x194
)
: ((x26 == 1) ? (
   x201*x304 - x304*x194 + x304*x196 + x304*x198
)
: ((x32 == 1) ? (
   -0.641731208021339*(x203*x307 + x205*x306 + x208*x305)
)
: ((x35 == 1) ? (
   -0.641731208021339*(x211*x303 + x213*x303 + x214*x303)
)
: 0))));
    double x311 = x46*x310;
    double x312 = x[3]*x[4];
    double x313 = x300*x[4];
    double x314 = x106 + x108 + x110 + x114;
    double x315 = x314 + 3.0*((x111 == 1) ? (
   1 + x132
)
: 0);
    double x316 = x45*x315;
    double x317 = x311 + x316 + x286*x308 + x47*((x19 == 1) ? (
   x233*x303 + x235*x303 + x237*x303 + x50*x303
)
: ((x26 == 1) ? (
   x238*x304 + x239*x304 + x240*x304 + x50*x304
)
: ((x32 == 1) ? (
   -0.641731208021339*(-x241*x305 - x242*x306 - x243*x307)
)
: ((x35 == 1) ? (
   -0.641731208021339*(-x244*x303 - x245*x303 - x246*x303)
)
: 0)))) + x62*x309 + x85*(x162 + x166 + x231 + x169*x[4]) + x85*(31.9888*x300 - x144*x[3] + x145*x[3] - x285*x[5] - x312*x150 + x313*x140 + x313*x148);
    double x318 = -x217*x310 + x289*x308;
    double x319 = x127 + x318 - x44*x119*x[5];
    double x320 = x123 + x319;
    double x321 = 16.629*x[2];
    double x322 = x321*x174;
    double x323 = x42*x322;
    double x324 = 462.697239451104*x17;
    double x325 = x324*x190;
    double x326 = pow(x181, 2);
    double x327 = 0.612033385517333*x88;
    double x328 = x327/pow(x7, 11);
    double x329 = 61.6929652601472*x88;
    double x330 = x36*x329;
    double x331 = 0.3672200313104*x90;
    double x332 = x331/pow(x7, 17);
    double x333 = 23.1348619725552*x90;
    double x334 = x333*x183;
    double x335 = 1.836100156552*x17;
    double x336 = x335/x33;
    double x337 = 912.541777806344*x74;
    double x338 = x36*x337;
    double x339 = pow(x193, 2);
    double x340 = x335/x27;
    double x341 = x200*x324;
    double x342 = x327/pow(x20, 11);
    double x343 = x331/pow(x20, 17);
    double x344 = x333*x195;
    double x345 = 2*x92;
    double x346 = x21*x345;
    double x347 = x92*x199;
    double x348 = (2.0/3.0)*x94;
    double x349 = pow(x20, 13)*x348;
    double x350 = 48.0*x94;
    double x351 = x206*x350;
    double x352 = (2.0/5.0)*x96;
    double x353 = pow(x20, 23)*x352;
    double x354 = 16.8*x96;
    double x355 = x202*x354;
    double x356 = x8*x345;
    double x357 = x92*x189;
    double x358 = pow(x7, 13)*x348;
    double x359 = pow(x7, 23)*x352;
    double x360 = x212*x350;
    double x361 = x210*x354;
    double x362 = pow(x71, -1);
    double x363 = 74092.0*x362;
    double x364 = pow(x[2], -9.0);
    double x365 = 10083.0 - 4.813*x[2] + ((x82 == 1) ? (
   -7976.15 + 137.093038*x[2] - 0.001884662*x155 + x363 - 8.77664e-07*x72 - x[2]*x154
)
: ((x83 == 1) ? (
   -11276.24 + 223.048446*x[2] + 0.018531982*x155 + x363 - 5.764227e-06*x72 - x[2]*x158
)
: ((x84 == 1) ? (
   -11278.378 + 188.684153*x[2] - 1.230524e+28*x364 - x[2]*x160
)
: 0)));
    double x366 = x365*x[7];
    double x367 = x231 + x366;
    double x368 = 2.0*x367;
    double x369 = 29249.2599*(x[5] + x138);
    double x370 = x36*x369;
    double x371 = 160000.0 - 1.0*x[2];
    double x372 = x371*x[6];
    double x373 = -103924.225 + 69.9194286*x[2];
    double x374 = x373*x147;
    double x375 = x36*x374;
    double x376 = -267841.207 + 131.456548*x[2];
    double x377 = x376*x139;
    double x378 = x36*x377;
    double x379 = 4.0*(-30740.0 + 7.9972*x[2]);
    double x380 = x379*x[7];
    double x381 = 9749.7533*x[4];
    double x382 = x381*x141;
    double x383 = -73554.0 + 4.0*x[2];
    double x384 = x383*x[7];
    double x385 = 51500.0 - 11.84*x[2];
    double x386 = x385*x[4];
    double x387 = x2*x386;
    double x388 = 3089.2*x2;
    double x389 = x388*x[5];
    double x390 = (2.0/3.0)*x376;
    double x391 = (1.0/3.0)*x141;
    double x392 = x373*x391;
    double x393 = -x392*x[4];
    double x394 = x386*x[7];
    double x395 = 3089.2*x[5];
    double x396 = x395*x[7];
    double x397 = x382 + x387 + x389 + x393 + x1*x396 - x370*x[4] + x372*x[7] + x375*x[4] + x378*x[4] + x380*x[5] + x384*x[4] + x390*x142 + x394*x149;
    double x398 = 2.0*x397;
    double x399 = (2.0/3.0)*x373;
    double x400 = x224*x399;
    double x401 = 19499.5066*x[4];
    double x402 = x36*x401;
    double x403 = pow(x43, -3);
    double x404 = 4017.0 - 1.255*x[2] + ((x75 == 1) ? (
   -7770.458 + 130.485235*x[2] - 0.00265684*x155 + 52478.0*x362 + 1.29223e-07*x72 - x[2]*x167
)
: ((x77 == 1) ? (
   -13542.026 + 183.803828*x[2] + 3.64167e+29*x364 - x[2]*x168
)
: 0));
    double x405 = x404*x[4];
    double x406 = ((x79 == 1) ? (
   1225.7 + 124.134*x[2] - 0.00439752*x155 + 77359.0*x362 - 5.8927e-08*x72 - x[2]*x163
)
: ((x80 == 1) ? (
   -25383.581 + 299.31255*x[2] + 2.29603e+31*x364 - x[2]*x164
)
: 0));
    double x407 = x406*x[7];
    double x408 = 2.0*(x366*x[3] + x405*x[7] + x407*x[5]);
    double x409 = 100000.0*x[4];
    double x410 = x409*x[7];
    double x411 = x1*x388;
    double x412 = 150000.0*x[5];
    double x413 = x412*x[6];
    double x414 = x386*x149;
    double x415 = x383*x[3];
    double x416 = x374*x[4];
    double x417 = x369*x[4];
    double x418 = x379*x[5];
    double x419 = 39258.0 - 4.14983*x[2];
    double x420 = x419*x[4];
    double x421 = x2*x377;
    double x422 = x421*x[4];
    double x423 = 2.0*(x2*x372 + x2*x414 + x2*x418 + x36*x420 + x410*x[6] + x411*x[5] + x413*x[7] + x416*x141 - x417*x141 + x422*x[5] + x78*x415);
    double x424 = x321*x136;
    double x425 = x403*x408 + x403*x423 + x403*x424 + x403*x250*x321;
    double x426 = x425 + x[2]*x101*x175;
    double x427 = x[2]*x104;
    double x428 = x[2]*x103;
    double x429 = -x250*x322 - x408*x174 - x423*x174 - x424*x174;
    double x430 = x429 - x[2]*x102;
    double x431 = x385*x149;
    double x432 = -x392;
    double x433 = x390*x141;
    double x434 = x2*x385;
    double x435 = (1.0/3.0)*x224;
    double x436 = -x435*x376;
    double x437 = x226 + x231;
    double x438 = x128 + x218 + x260;
    double x439 = -x215*x252 - x219*x251 - x248*x367 - x248*x397;
    double x440 = x42*x258;
    double x441 = -x440*x251;
    double x442 = x231 + x404*x[7];
    double x443 = x376*x391;
    double x444 = -x443*x[4];
    double x445 = x419*x[5];
    double x446 = 100000.0*x[6];
    double x447 = x382 - x387 + x444 - x369*x141 + x374*x141 + x399*x142 + x415*x[7] + x421*x[5] + x434*x149 + x445*x[7] + x446*x[7];
    double x448 = x441 - x251*x256 - x442*x248 - x447*x248;
    double x449 = x[2]*x257 + x85*x442 + x85*x447;
    double x450 = x[2]*x220 + x[2]*x221 + x85*x367 + x85*x397;
    double x451 = x263 + x430 + x449 + x450 + x130*(x265 + x426 + x438 + x439 + x448 + x85*(9749.7533*x141 - x370 + x375 + x378 + x384 - x394 + x400 + x432 + x433 + x434 + x436 + x36*x381 + x431*x[7]) + (x230 + x437)*x85);
    double x452 = x328*x181;
    double x453 = 504.0*x1;
    double x454 = x453*x[7];
    double x455 = x177 - x3 - x454;
    double x456 = x268*x181;
    double x457 = x270*x193;
    double x458 = -x177 + x3 + x454;
    double x459 = x208*x199;
    double x460 = x205*x206;
    double x461 = x268*x359;
    double x462 = x2*x381;
    double x463 = (1.0/3.0)*x222;
    double x464 = 3089.2*x1;
    double x465 = x222*x390;
    double x466 = x227*x[7];
    double x467 = x230 + x231;
    double x468 = 19499.5066*x141;
    double x469 = 150000.0*x[6];
    double x470 = -x389 + x393 + x411 + x422 + x444 + x2*x379 - x2*x417 + x222*x374 + x420*x[7] - x468*x[4] + x469*x[7];
    double x471 = x231 + x407;
    double x472 = x99*x282;
    double x473 = x425 - x275*x252 - x279*x251 - x470*x248 - x471*x248 - x472*x251;
    double x474 = x439 + x473 + x215*x284 + x47*((x19 == 1) ? (
   -x452*x268 + x455*x185 + x455*x188 + x455*x192 - x455*x194 - x456*x332 - x456*x336
)
: ((x26 == 1) ? (
   -x457*x340 - x457*x342 - x457*x343 - x458*x194 + x458*x196 + x458*x198 + x458*x201
)
: ((x32 == 1) ? (
   -0.641731208021339*(x457*x346 + x457*x349 + x457*x353 + x458*x204 + x458*x460 + x459*x458)
)
: ((x35 == 1) ? (
   -0.641731208021339*(x455*x211 + x455*x213 + x455*x214 + x456*x356 + x456*x358 + x461*x181)
)
: 0)))) + x85*(x380 + x388 - x396 - x402 + x436 + x462 + x465 - x435*x373 - x463*x373 + x464*x[7] - x78*x369 + x78*x374 + x78*x377) + (x466 + x467)*x85;
    double x475 = x99*x121;
    double x476 = x429 + x[2]*x276 + x[2]*x280 + x475*x282 + x85*x470 + x85*x471;
    double x477 = x120 + x450 + x476;
    double x478 = x371*x[3];
    double x479 = x410 + x412*x[7] + x478*x[7];
    double x480 = x441 - x231*x248 - x294*x251 - x479*x248;
    double x481 = x266 + x480;
    double x482 = x130*(x438 + x481 + x85*x467 + x85*x371*x[7]);
    double x483 = x296 + x[2]*x295 + x85*x479;
    double x484 = (-1.0/3.0)*x373*x313;
    double x485 = x395*x[3];
    double x486 = x386*x[3];
    double x487 = x381*x300;
    double x488 = x377*x[4];
    double x489 = x376*x313;
    double x490 = x225*x[4];
    double x491 = x229*x[3];
    double x492 = x228 + x490 + x491;
    double x493 = x303*x181;
    double x494 = -x301;
    double x495 = x453*x[5];
    double x496 = x494 - x495;
    double x497 = x304*x193;
    double x498 = x301 + x495;
    double x499 = x379*x[3];
    double x500 = x413 + x1*x485 + x372*x[3] + x409*x[6] + x415*x[4] + x416*x300 - x417*x300 + x420*x[5] + x486*x149 + x488*x300 + x499*x[5];
    double x501 = x231 + x405 + x365*x[3] + x406*x[5];
    double x502 = -x251*x315 - x252*x310 - x500*x248 - x501*x248 - x99*x251*x308;
    double x503 = x502 + x215*x309 + x47*((x19 == 1) ? (
   -x452*x303 - x493*x332 - x493*x336 + x496*x185 + x496*x188 + x496*x192 - x496*x194
)
: ((x26 == 1) ? (
   -x497*x340 - x497*x342 - x497*x343 - x498*x194 + x498*x196 + x498*x198 + x498*x201
)
: ((x32 == 1) ? (
   -0.641731208021339*(x497*x346 + x497*x349 + x497*x353 + x498*x204 + x498*x459 + x498*x460)
)
: ((x35 == 1) ? (
   -0.641731208021339*(x493*x356 + x493*x358 + x493*x359 + x496*x211 + x496*x213 + x496*x214)
)
: 0)))) + x85*(x365 + x467 + x492) + x85*(x372 + x414 + x418 + x484 + x485 + x486 + x487 + (2.0/3.0)*x489 + x1*x395 + x383*x[4] + x416*x[5] - x417*x[5] + x488*x[5]);
    double x504 = x[2]*x311 + x[2]*x316 + x475*x308 + x85*x500 + x85*x501;
    double x505 = 2.0*x442;
    double x506 = 2.0*x447;
    double x507 = -x63*x259;
    double x508 = x128 + x507;
    double x509 = x284*x258;
    double x510 = x265 + x291 + x509;
    double x511 = x222*x399;
    double x512 = x448 + x473 + x85*(x421 + x432 - x443 + x462 - x468 + x511 - x2*x369 + x2*x374 + x419*x[7] - x463*x376) + (x437 + x466)*x85;
    double x513 = x264 + x449 + x476;
    double x514 = x130*(100000.0*x288 + x481 + x508 + x85*x437);
    double x515 = x265 + x319;
    double x516 = x258*x309;
    double x517 = (-1.0/3.0)*x489;
    double x518 = x502 + x516 + x85*(x404 + x437 + x492) + x85*(x415 + x445 + x446 - x486 + x487 + x517 - x369*x300 + x374*x300 + x377*x300 + x399*x313 + x431*x[3]);
    double x519 = 2.22*x66;
    double x520 = -x519*x[7];
    double x521 = 4.44*x126 + x281;
    double x522 = x290 + (x520 + x521)*x475;
    double x523 = x122 + x522;
    double x524 = x122 + x263;
    double x525 = x509 + x522 + x524;
    double x526 = pow(x268, 2);
    double x527 = x2*x329;
    double x528 = x2*x337;
    double x529 = pow(x270, 2);
    double x530 = 2.0*x471;
    double x531 = x64*x428;
    double x532 = 2.0*x470;
    double x533 = 4.44*x66;
    double x534 = x121*x125;
    double x535 = x231 + x466;
    double x536 = 150000.0*x288 + x480 + x85*x535;
    double x537 = x268*x303;
    double x538 = x453*x[3];
    double x539 = -1043.0 + x301 - x538;
    double x540 = 1043.0 + x494 + x538;
    double x541 = x270*x304;
    double x542 = x521 - x519*x[5];
    double x543 = x130*(x122 + x502 + x275*x309 + x284*x310 + x47*((x19 == 1) ? (
   -x537*x328 - x537*x332 - x537*x336 + x539*x185 + x539*x188 + x539*x192 - x539*x194
)
: ((x26 == 1) ? (
   -x540*x194 + x540*x196 + x540*x198 + x540*x201 - x541*x340 - x541*x342 - x541*x343
)
: ((x32 == 1) ? (
   -0.641731208021339*(x459*x540 + x460*x540 + x540*x204 + x541*x346 + x541*x349 + x541*x353)
)
: ((x35 == 1) ? (
   -0.641731208021339*(x461*x303 + x537*x356 + x537*x358 + x539*x211 + x539*x213 + x539*x214)
)
: 0)))) + x85*(x406 + x492 + x535) + x85*(x420 + x469 + x484 - x485 + x499 + x517 + x374*x312 - x401*x300 - x417*x[3] + x464*x[3] + x488*x[3]) + (x40 + x520 + x542)*x475 - x534*x282*x308);
    double x544 = 1.0*x483;
    double x545 = x516 + x85*(x409 + x412 + x478) + (x231 + x492)*x85;
    double x546 = x318 + x475*x542;
    double x547 = x122 + x546;
    double x548 = 1.0*x504;
    double x549 = x524 + x546;
    double x550 = pow(x303, 2);
    double x551 = pow(x304, 2);
    out[0] = x130*(-x102 + x129 + x103*x117 + x47*((x19 == 1) ? (
   -x13*x11 - x15*x16 - x7*x18 - x9*x10
)
: ((x26 == 1) ? (
   -x20*x18 - x22*x10 - x24*x16 - x25*x13
)
: ((x32 == 1) ? (
   -0.641731208021339*(x23*x29 + x28*x27 + x30*x31)
)
: ((x35 == 1) ? (
   -0.641731208021339*(x29*x14 + x31*x34 + x33*x28)
)
: 0)))) + x62*x104 - x70*x62 + x85*(x2*((x82 == 1) ? (
   -0.003769324 - 5.265984e-06*x71 - 24.3671976*x74 + x81
)
: ((x83 == 1) ? (
   0.037063964 - 3.4585362e-05*x71 - 38.5844296*x74 + x81
)
: ((x84 == 1) ? (
   -31.748192*x74 - 1.1074716e+30*x76
)
: 0))) + x36*((x79 == 1) ? (
   -0.00879504 - 3.53562e-07*x71 + 154718.0*x73 - 23.5143*x74
)
: ((x80 == 1) ? (
   -46.0*x74 + 2.066427e+33*x76
)
: 0)) + x78*((x75 == 1) ? (
   -0.00531368 + 7.75338e-07*x71 + 104956.0*x73 - 24.112392*x74
)
: ((x77 == 1) ? (
   -31.38*x74 + 3.277503e+31*x76
)
: 0))));
    out[1] = x173 + x254;
    out[2] = x173 + x267;
    out[3] = x173 + (x287 + x292)*x130;
    out[4] = x298;
    out[5] = (x317 + x320)*x130;
    out[6] = x172 + x254;
    out[7] = x430 + x130*(x120 + x128 + x426 + x121*(x116 + 1.0*((x105 == 1) ? (
   pow(x[3], -1)
)
: 0)) - x215*x323 - x219*x322 - x368*x174 - x398*x174 + x47*((x19 == 1) ? (
   x338 - x326*x328 - x326*x332 - x330*x186 - x336*x326 - x36*x325 - x36*x334
)
: ((x26 == 1) ? (
   -x338 + x330*x197 - x339*x340 - x339*x342 - x339*x343 + x36*x341 + x36*x344
)
: ((x32 == 1) ? (
   -0.641731208021339*(x339*x346 + x339*x349 + x347*x177 + x353*x339 + x36*x351 + x36*x355)
)
: ((x35 == 1) ? (
   -0.641731208021339*(x356*x326 - x357*x177 + x358*x326 + x359*x326 - x36*x360 - x36*x361)
)
: 0)))) - x70*x215 + x85*(6178.4*x36 + 2*x394 - x400 + x402 + (4.0/3.0)*x224*x376) + (2*x230 + x231)*x85) + x427*x215 + x428*x219 + x44*x368 + x44*x398;
    out[8] = x451;
    out[9] = x477 + (x292 + x474)*x130;
    out[10] = x482 + x483;
    out[11] = x504 + (x320 + x503)*x130;
    out[12] = x172 + x267;
    out[13] = x451;
    out[14] = x430 + x130*(x264 + x426 + x508 + x121*(x255 + 1.0*((x107 == 1) ? (
   pow(x[4], -1)
)
: 0)) - x256*x322 - x440*x322 - x505*x174 - x506*x174 + x85*(-x433 - 2*x434 + x468 + (4.0/3.0)*x373*x141) + (2*x226 + x231)*x85) + x427*x258 + x428*x256 + x44*x505 + x44*x506;
    out[15] = x513 + (x510 + x512)*x130;
    out[16] = x483 + x514;
    out[17] = x504 + (x515 + x518)*x130;
    out[18] = x172 + (x287 + x523)*x130;
    out[19] = x477 + (x474 + x523)*x130;
    out[20] = x513 + (x512 + x525)*x130;
    out[21] = x429 + x130*(x425 + x121*(x278 + 1.0*((x109 == 1) ? (
   pow(x[5], -1)
)
: 0)) - x275*x323 - x279*x322 + x47*((x19 == 1) ? (
   -x528 + x2*x325 + x2*x334 - x526*x328 - x526*x332 - x526*x336 + x527*x186
)
: ((x26 == 1) ? (
   x528 - x2*x341 - x2*x344 - x527*x197 - x529*x340 - x529*x342 - x529*x343
)
: ((x32 == 1) ? (
   -0.641731208021339*(-x2*x351 - x2*x355 - x3*x347 + x529*x346 + x529*x349 + x529*x353)
)
: ((x35 == 1) ? (
   -0.641731208021339*(x2*x360 + x2*x361 + x3*x357 + x526*x356 + x526*x358 + x526*x359)
)
: 0)))) - x472*x322 + x475*(x521 - x533*x[7]) - x530*x174 - x532*x174 - x534*pow(x282, 2) + x85*(-6178.4*x2 - 38999.0132*x222 - x465 - x511) + (x231 + 2*x466)*x85 + x531*x275*x282) + x427*x275 + x428*x279 + x44*x530 + x44*x532 + x472*x428;
    out[22] = x483 + (x525 + x536)*x130;
    out[23] = x504 + x543;
    out[24] = x298;
    out[25] = x482 + x544;
    out[26] = x514 + x544;
    out[27] = x544 + (x510 + x536)*x130;
    out[28] = x130*(x297 + x507 + x121*(x293 + 1.0*((x113 == 1) ? (
   pow(x[6], -1)
)
: 0)));
    out[29] = (x515 + x545)*x130;
    out[30] = (x317 + x547)*x130;
    out[31] = x548 + (x503 + x547)*x130;
    out[32] = x548 + (x518 + x549)*x130;
    out[33] = x543 + x548;
    out[34] = (x545 + x549)*x130;
    out[35] = x130*(x121*(x314 + 3.0*((x111 == 1) ? (
   pow(x[7], -1)
)
: 0)) + x47*((x19 == 1) ? (
   -x550*x328 - x550*x332 - x550*x336
)
: ((x26 == 1) ? (
   -x551*x340 - x551*x342 - x551*x343
)
: ((x32 == 1) ? (
   -0.641731208021339*(x551*x346 + x551*x349 + x551*x353)
)
: ((x35 == 1) ? (
   -0.641731208021339*(x550*x356 + x550*x358 + x550*x359)
)
: 0)))) + x475*(x521 - x533*x[5]) - x534*pow(x308, 2) + (2*x228 + x231 + 2*x490 + 2*x491)*x85 + x531*x308*x310);
}
...
__global__ void init_all_gpu_phase_records() {
    ...
    #endif
        g_phase_records_array[0].init(&pycgpu_model_0_obj, &pycgpu_model_0_formulaobj, &pycgpu_model_0_formulagrad, &pycgpu_model_0_formulahess, &pycgpu_model_0_internal_cons_func, &pycgpu_model_0_internal_cons_jac, &pycgpu_model_0_mass_obj, &pycgpu_model_0_formulamole_obj, &pycgpu_model_0_formulamole_grad, 3, 5, 4, 2, 3);
    g_phase_records_array[1].init(&pycgpu_model_1_obj, &pycgpu_model_1_formulaobj, &pycgpu_model_1_formulagrad, &pycgpu_model_1_formulahess, &pycgpu_model_1_internal_cons_func, &pycgpu_model_1_internal_cons_jac, &pycgpu_model_1_mass_obj, &pycgpu_model_1_formulamole_obj, &pycgpu_model_1_formulamole_grad, 3, 3, 4, 1, 3);
