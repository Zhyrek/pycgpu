...
typedef struct SystemState {
    ...
    __device__ void init(SystemSpecification* spec, CompositionSet* initial_compsets, int initial_num_compsets) {
        ...
        for (int i = 0; i < num_compsets; ++i) {
            // Check compset[i]
            if (!compsets[i].fixed && compsets[i].NP > MIN_PHASE_FRACTION) { // Use same threshold as CPU
    ...
    __device__ void recompute(SystemSpecification* spec) {
        ...
        if (thread_id == 0 && idx < 2 && pr->phase_dof > 0) {
            ...
            if (thread_id == 0 && iteration < 2) {
                printf("[GPU DEBUG] Phase %d mass_jac BEFORE c_component calc (phase_dof=%d, num_elements=%d, nonvacant=%d):\n", 
                       idx, pr->phase_dof, pr->num_elements, pr->nonvacant_elements);
            ...
            for (int cidx = 0; cidx < spec->num_components; ++cidx) {
                for (int i = 0; i < num_phase_dof_for_csst; ++i) {
                    for (int j = 0; j < num_phase_dof_for_csst; ++j) {
                         // CRITICAL FIX: Only calculate c_component for non-vacant elements
                         // CPU uses num_nonvacant_components, not all components including VA
                         // Check if this component is VA by checking if mass is always 0
                         if (cidx < pr->nonvacant_elements) { // Use nonvacant_elements (3), not num_elements (4)!
                            #ifdef VERBOSE_DEBUG
                            if (thread_id == 0 && iteration == 0 && cidx == 3) {
                                printf("[GPU DEBUG] ERROR: Calculating c_component for VA (cidx=3)! This should not happen!\n");
                            }
                            #endif
...
__device__ void fill_equilibrium_system(double* equilibrium_matrix, int equilibrium_matrix_cols,
    ...
    for (int free_idx = 0; free_idx < state->num_free_stable_compsets; free_idx++) {
        ...
        for (int mole_frac_cond_row_idx = 0; mole_frac_cond_row_idx < num_fixed_mole_frac_conds; mole_frac_cond_row_idx++) {
            for (current_component_idx = 0; current_component_idx < spec->num_prescribed_mole_fraction_coefficients_cols; current_component_idx++) {
                ...
                // CRITICAL FIX: Match CPU exactly - only call write_row_fixed_mole_fraction if prefactor is non-zero
                if (fabs(prefactor) < 1e-12) continue;  // Skip if prefactor is zero, like CPU does
    ...
    for (int fixed_idx = 0; fixed_idx < spec->num_fixed_stable_compsets; fixed_idx++) {
        ...
        for (int mole_frac_cond_row_idx = 0; mole_frac_cond_row_idx < num_fixed_mole_frac_conds; mole_frac_cond_row_idx++) {
            for (current_component_idx = 0; current_component_idx < spec->num_prescribed_mole_fraction_coefficients_cols; current_component_idx++) {
                prefactor = spec->prescribed_mole_fraction_coefficients[mole_frac_cond_row_idx][current_component_idx];
                // CRITICAL FIX: Match CPU exactly - only call if prefactor is non-zero
...
: (
   0
    ...
    out[1] = x262 + x0*(x259 + x108*(-x152 - x153*x[11] + x156*x[10]) + x108*(x146*x[8] + x148*x[8] + x149*x150) + x143*(x139 + x144 + 0.5*((x117 == 1) ? (
   1 + x116
)
: (
   0
...
: (
   0
    ...
    out[3] = x262 + x0*(x281 + x108*(x239 + 54.1229146*x242*x[7] + x273*x150) + x143*(x138 + x144 + 0.5*((x115 == 1) ? (
   1 + x114
)
: (
   0
...
: 0))));
    ...
    double x150 = x1*x[11];
    double x151 = log(x[3]);
    double x152 = x11 + x15 + x17 + x3 + x5 + x7 + x9;
    double x153 = x152 + x19;
    double x154 = x153 + 0.5*((x12 == 1) ? (
   1 + x151
)
: 0);
...
: 0))) - x70*x90);
    ...
    out[9] = (x387 + x391)*x0;
    out[10] = x299 + x396;
    out[11] = x560 + x562 + x0*(x546 + x145*(x153 + 0.5*((x12 == 1) ? (
   pow(x[3], -1)
)
