#!/usr/bin/env python
"""Comprehensive GPU vs CPU comparison for FCC_A1 + LIQUID phases in Au-Bi system."""
...
def run_comprehensive_test(verbose=False):
    ...
    comps = ['AU', 'BI', 'VA']
    # Only test FCC_A1 and LIQUID phases - FCC_A1 has vacancy sublattice
    ...
    conditions = {
        ...
        v.T: (400, 900, 100),        # 400 to 900 in 100 increments  
        v.P: 101325
    }
    ...
    temperatures = np.arange(400, 901, 100)  # This gives [400, 500, ..., 900]
    expected_conditions = 8 * 6  # 8 compositions * 6 temperatures = 48
    
    print(f"Testing Au-Bi system with FCC_A1 + LIQUID phases")
    print(f"FCC_A1 has vacancy sublattice: (AU,BI)(VA)")
    ...
    try:
        ...
        # Open output file
        with open('gpu_cpu_aubi_fcc_liquid_results_multi.txt', 'w') as f:
            # Write header
            ...
            for i in range(num_results):
                ...
                f.write(f"{cpu_mu_au_flat[i]:.12f}\t{cpu_mu_bi_flat[i]:.12f}\t{gpu_mu_au_flat[i]:.12f}\t{gpu_mu_bi_flat[i]:.12f}\t")
        ...
        print(f"Results saved to gpu_cpu_aubi_fcc_liquid_results_multi.txt")
        print(f"\nSummary:")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Run comprehensive GPU vs CPU comparison test for Au-Bi FCC+LIQUID')
