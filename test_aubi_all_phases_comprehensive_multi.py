#!/usr/bin/env python
"""Comprehensive GPU vs CPU comparison for AuBi system with all phases enabled using multiple conditions per call."""
...
def run_comprehensive_test(verbose=False):
    """Run comprehensive GPU vs CPU comparison test for AuBi system with all phases."""
    
    # Load database and set up calculation
    dbf = Database('important_tests/AuBi-07Wan.tdb')
    comps = ['AU', 'BI', 'VA']
    phases = filter_phases(dbf, comps)  # This will include all phases in the database
    ...
    temperatures = np.arange(400, 801, 100)  # This gives [400, 500, 600, 700, 800]
    ...
    try:
        ...
        # Open output file
        with open('gpu_cpu_aubi_all_phases_results_multi.txt', 'w') as f:
            # Write header
        ...
        print(f"Results saved to gpu_cpu_aubi_all_phases_results_multi.txt")
        print(f"\nSummary:")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Run comprehensive GPU vs CPU comparison test for AuBi system with all phases')
