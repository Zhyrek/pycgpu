#!/usr/bin/env python
"""Final test of dgelsd implementation with clean output."""
...
for T in temperatures:
    for x_ti in ti_fractions:
        ...
        try:
            ...
            cpu_gm = float(cpu_result.GM.values.item())  # Use .item() to get scalar
            
            # GPU calculation  
            gpu_result = equilibrium(dbf, comps, phases, conditions, 
                                   calc_opts={'pdens': 100}, verbose=False, gpu=True)
            gpu_gm = float(gpu_result.GM.values.item())  # Use .item() to get scalar
...
if results:
    ...
    if max_diff < 6e-4:
        ...
        print(f"  Current maximum error: {max_diff:.1e} J/mol")