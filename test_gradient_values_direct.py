#!/usr/bin/env python
"""Test gradient values directly from generated functions."""
...
extern "C" __global__ void test_gradient_kernel(double* output) {
    if (threadIdx.x == 0 && blockIdx.x == 0) {
        ...
        printf("GPU Gradient output:\\n");
        printf("  grad[0] (T derivative) = %.10f\\n", grad[0]);
        printf("  grad[1] (Y_NB derivative) = %.10f\\n", grad[1]);
        printf("  grad[2] (Y_TI derivative) = %.10f\\n", grad[2]);
...
mod = cp.RawModule(code=test_kernel_code, options=['-std=c++11'])
