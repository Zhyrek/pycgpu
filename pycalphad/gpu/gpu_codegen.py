...
def notebook_get_all_sym_names_for_model(model_obj: Model, wks_obj: Workspace) -> list:
    ...
    # This should match exactly what the CPU expects:
    # For BCC_A2: ['N', 'P', 'T'] + ['BCC_A20NB', 'BCC_A20TI']
...
__device__ void solve_equilibrium_at_condition_global_mem(
    ...
    if (!A_lstsq_copy || !result || !global_spec_base || !initial_data) {{
        ...
        return; // Cannot proceed without required arrays
    }}
    ...
    SystemSpecification current_spec = *global_spec_base;
    ...
    for (int i = 0; i < num_phases && i < MAX_PHASES; ++i) {{
        ...
        if (current_sys_state.num_compsets < MAX_PHASES) {{
            ...
            for (int j = 0; j < jac_size && j < (MAX_COMPONENTS * (MAX_STATEVARS + MAX_DOF_PER_PHASE)); ++j) {{
                css->mass_jac[j] = 0.0;
            }}
        }}
        ...
        if (cs->phase_record && cs->phase_record->formulamole_obj) {{
            ...
            // Model expects [T, Y1, Y2...] but we have [N, P, T, Y1, Y2...]
            double model_dof[MAX_STATEVARS + MAX_DOF_PER_PHASE];
            
            // Extract temperature from workspace state variables
            if (cs->phase_record->num_statevars == 1) {{
                // Model expects just T
                if (current_spec.num_statevars == 2) {{
                    // Workspace has [N, T], so T is at index 1
                    model_dof[0] = cs->dof[1];
                }} else if (current_spec.num_statevars >= 3) {{
                    // Workspace has [N, P, T], so T is at index 2
                    model_dof[0] = cs->dof[2];
                }} else {{
                    // Fallback
                    model_dof[0] = 298.15;
                }}
            ...
            for (int sf_idx = 0; sf_idx < cs->phase_record->phase_dof; ++sf_idx) {{
                model_dof[cs->phase_record->num_statevars + sf_idx] = cs->dof[current_spec.num_statevars + sf_idx];
            }}
            
            cs->phase_record->formulamole_obj(masses_tmp, model_dof);
    ...
    for (int idx = 0; idx < current_sys_state.num_compsets; ++idx) {{
        ...
        // CRITICAL FIX: Create Model DOF from Workspace DOF before calling formulamole_obj
        double model_dof_temp[MAX_STATEVARS + MAX_DOF_PER_PHASE];
        if (cs->phase_record->num_statevars == 1) {{
            // Extract temperature from workspace state variables
            if (current_spec.num_statevars == 2) {{
                model_dof_temp[0] = cs->dof[1];  // [N, T]
            }} else if (current_spec.num_statevars >= 3) {{
                model_dof_temp[0] = cs->dof[2];  // [N, P, T]
            }}
        }}
        // Copy site fractions
        for (int sf_idx = 0; sf_idx < cs->phase_record->phase_dof; ++sf_idx) {{
            model_dof_temp[cs->phase_record->num_statevars + sf_idx] = cs->dof[current_spec.num_statevars + sf_idx];
        }}
        ...
        if (cs->phase_record->formulamole_obj) {{
            ...
            cs->phase_record->formulamole_obj(formulamoles, model_dof_temp);
        }} else {{
            if (thread_id == 0) {{
    ...
    for (int i = 0; i < current_sys_state.num_compsets; ++i) {{
        ...
        // Simple energy calculation without complex formula unit conversion
        double current_dof_for_phase[MAX_STATEVARS + MAX_DOF_PER_PHASE];
        // Initialize entire array to zero to avoid garbage values
        for(int k=0; k < MAX_STATEVARS + MAX_DOF_PER_PHASE; ++k) current_dof_for_phase[k] = 0.0;
        ...
        // CRITICAL FIX: Create Model DOF from Workspace DOF
        // The Model functions expect [T, Y_NB, Y_TI] format, not workspace [N, P, T, Y1, Y2] format
        // Extract temperature from workspace state variables
        if (cs->phase_record->num_statevars == 1) {{
            // Model expects just T
            if (current_spec.num_statevars == 2) {{
                // Workspace has [N, T], so T is at index 1
                current_dof_for_phase[0] = cs->dof[1];
            }} else if (current_spec.num_statevars >= 3) {{
                // Workspace has [N, P, T], so T is at index 2
                current_dof_for_phase[0] = cs->dof[2];
            }} else {{
                // Fallback
                current_dof_for_phase[0] = 298.15;
            }}
        }}
        
        // Copy site fractions from workspace DOF
        for (int sf_idx = 0; sf_idx < cs->phase_record->phase_dof; ++sf_idx) {{
            current_dof_for_phase[cs->phase_record->num_statevars + sf_idx] = cs->dof[current_spec.num_statevars + sf_idx];
        }}
        
        // DEBUG: Check DOF values before energy calculation
        if (thread_id == 0) {{
            printf("GPU DEBUG: Phase %d - DOF for energy calc (Model format): [", i);
            int num_model_vars = cs->phase_record->num_statevars + cs->phase_record->phase_dof;
            for(int k=0; k < num_model_vars; ++k) {{
                printf("%.15f", current_dof_for_phase[k]);
                if (k < num_model_vars - 1) printf(", ");
            }}
            printf("] (T, Y(NB), Y(TI)...)\\n");
            printf("GPU DEBUG: Phase %d - Expected: %d model_statevars + %d phase_dof = %d total\\n", 
                   i, cs->phase_record->num_statevars, cs->phase_record->phase_dof, num_model_vars);
        }}
        
        // Calculate energy using the same function as the first calculation (obj, not formulaobj)
        css->energy = cs->phase_record->obj(current_dof_for_phase);
        
        // DEBUG: Check energy result - energies should be negative for this system!
        if (thread_id == 0) {{
            int num_model_vars = cs->phase_record->num_statevars + cs->phase_record->phase_dof;
            printf("[GPU DEBUG] Phase %d energy result = %.6f J/mol\\n", i, css->energy);
            printf("[GPU DEBUG] Phase %d DOF values (Model format): ", i);
            for (int k = 0; k < num_model_vars; ++k) {{
                printf("%.15f ", current_dof_for_phase[k]);
            }}
            printf("\\n");
            printf("[GPU DEBUG] Phase %d: T=%.3f, Y(NB)=%.15f, Y(TI)=%.15f, energy=%.6f\\n", 
                   i, current_dof_for_phase[0], 
                   (num_model_vars > 1 ? current_dof_for_phase[1] : 0.0), 
                   (num_model_vars > 2 ? current_dof_for_phase[2] : 0.0), 
                   css->energy);
...
__global__ void top_level_equilibrium_kernel(
    ...
    if (tid < num_conditions_total && results_list_ptr_raw != nullptr) {{
        ...
        const SystemSpecification* sys_spec = (const SystemSpecification*)global_spec_ptr_raw;
        ...
        if (initial_data_byte_array != nullptr && condition_idx < num_conditions_total) {{
            ...
            int actual_num_statevars = sys_spec->num_statevars;
            ...
            SystemSpecification thread_spec = *sys_spec;  // Copy global spec
            ...
            if (condition_idx == 0) {{
                ...
                printf("GPU DEBUG: sys_spec=%p, thread arrays ready\\n", sys_spec);
            }}
