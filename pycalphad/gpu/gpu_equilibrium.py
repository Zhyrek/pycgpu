...
def _prepare_gpu_data(wks_obj: Workspace, unique_py_models: list, py_phase_name_to_unique_idx_map: dict, dynamic_sizes: dict = None, properties=None, grid=None):
    ...
    for cond_idx in range(num_conditions_total):
        ...
        if len(phase_names_in_condition) != len(unique_phase_names):
            ...
            if wks_obj.verbose:
                print(f"[GPU] WARNING: Condition {cond_idx} has duplicate phase names: {phase_names_in_condition}")
                print(f"[GPU] This indicates an immiscibility gap. GPU calculation may be incorrect.")
                print(f"[GPU] Consider using CPU calculation for this case.")
...
def _process_gpu_results(results_cpu_flat: np.ndarray, wks_obj: Workspace,
    ...
    if results_cpu_flat.size > 0:
        ...
        if wks_obj.verbose:
            print("[GPU] Checking for symmetric composition phase merger...")
        
        # CRITICAL FIX: Disable incorrect phase merger that was collapsing miscibility gaps
        # The CPU doesn't merge phases just because their weighted average equals the constraint
        # In a miscibility gap, we SHOULD have two phases with different compositions
        ...
        # CRITICAL FIX: Completely disable phase merger - it was incorrectly collapsing miscibility gaps
        # The presence of X(TI)=0.5 constraint doesn't mean phases should be merged
        # The system can satisfy this constraint by having two phases with different compositions
        # whose weighted average equals 0.5
        has_symmetric_constraint = False  # Force disable merger
        symmetric_component_idx = None
        
        if False:  # Keep original code for reference but disabled
            for comp_idx, component in enumerate(non_va_components):
                ...
                if x_var in wks_obj.conditions:
                    ...
                    if abs(x_scalar - 0.5) < 1e-6:
                        ...
                        if wks_obj.verbose:
                            print(f"[GPU] Found symmetric constraint: X({component}) = {x_scalar}")
                            print(f"[GPU] Component index in non_va_components: {comp_idx}")
                            print(f"[GPU] All non-VA components: {non_va_components}")
                    break
        ...
        if has_symmetric_constraint and symmetric_component_idx is not None:
            ...
            for cond_idx in range(results_cpu_flat.size):
                ...
                if wks_obj.verbose:
                    print(f"[GPU] Condition {cond_idx} - Active phases: {active_phases}")
                    print(f"[GPU] Phase IDs: {phase_ids}")
                    print(f"[GPU] Phase amounts: {phase_amounts}")
                ...
                if len(active_phases) >= 2:
                    ...
                    if wks_obj.verbose:
                        print(f"[GPU] Phase types: {phase_types}")
                    ...
                    for phase_id, indices in phase_types.items():
                        if len(indices) >= 2:
                            ...
                            # DEBUG: Check if x2 is zero when it shouldn't be
                            if wks_obj.verbose and x2 == 0.0 and phase_amounts[idx2] > 1e-6:
                                print(f"[GPU] WARNING: Phase {idx2} has X({non_va_components[symmetric_component_idx]})=0 but amount={phase_amounts[idx2]:.6f}")
                                print(f"[GPU] Full phase_x[{idx2}]: {phase_x[idx2, :]}")
                                # Check the raw data
                                raw_start = idx2 * max_comps_kernel
                                print(f"[GPU] Raw x_flat data for phase {idx2}: {phase_x_flat[raw_start:raw_start+max_comps_kernel]}")
                            
                            if wks_obj.verbose:
                                print(f"[GPU] Phase {idx1} composition: {phase_x[idx1, :]}")
                                print(f"[GPU] Phase {idx2} composition: {phase_x[idx2, :]}")
                            ...
                            if wks_obj.verbose:
                                print(f"[GPU] Phase merger check: phase {idx1} (X={x1:.6f}, amt={amt1:.6f}) + "
                                      f"phase {idx2} (X={x2:.6f}, amt={amt2:.6f})")
                                print(f"[GPU] Total amount: {total_amt:.6f}, merged X would be: {(amt1 * x1 + amt2 * x2) / total_amt if total_amt > 1e-12 else 0:.6f}")
                            ...
                            if total_amt > 1e-12 and num_output_components == 2:
                                ...
                                if abs(amt1 - amt2) < 1e-6 and abs(amt1 - 0.5) < 1e-6:
                                    ...
                                    if wks_obj.verbose:
                                        print(f"[GPU] FORCING MERGER: Detected symmetric binary system with equal phase amounts")
                                    force_merge = True
                                else:
                            ...
                            if force_merge:
                                    if wks_obj.verbose:
                                        print(f"[GPU] Merging symmetric phases at condition {cond_idx}: "
                                              f"phase {idx1} (X={x1:.6f}, amt={amt1:.6f}) + "
                                              f"phase {idx2} (X={x2:.6f}, amt={amt2:.6f}) -> "
                                              f"merged (X=0.5, amt={total_amt:.6f})")
        ...
        if wks_obj.verbose:
            print("[GPU] Phase merger check complete.")
        ...
        try:
            ...
            if wks_obj.verbose:
                print(f"[GPU] WARNING: Reshape failed, using fallback reshape: {e}")
                print(f"[GPU] DEBUG: gm_flat shape = {gm_flat.shape}, output_shape = {output_shape}")
                print(f"[GPU] DEBUG: coords_keys_for_shape = {coords_keys_for_shape}")
            ...
            try:
                ...
                if gm_flat.size == np.prod(output_shape):
                    ...
                    print(f"[GPU] ERROR: Cannot reshape GM - size mismatch: {gm_flat.size} != {np.prod(output_shape)}")
                ...
                print(f"[GPU] CRITICAL ERROR in fallback reshape: {reshape_error}")
...
def calculate_equilibrium_gpu(wks_obj: Workspace, to_xarray=True, validate_code=False, force_cpu=False):
    ...
    try:
        ...
        if verbose:
            ...
            print(f"[GPU] DEBUG: Available functions in module:")
            try:
    ...
    # Test the GPU setup with a simple kernel first
    try:
        test_kernel = module.get_function("test_kernel")
        test_input = cp.arange(10, dtype=cp.float64)
        test_output = cp.zeros(10, dtype=cp.float64)
        test_kernel((1,), (10,), (test_output.data.ptr, test_input.data.ptr, 10))
        cp.cuda.runtime.deviceSynchronize()
    ...
    try:
        ...
        # Updated Layout: GM, chemical_potentials[MAX_COMPONENTS], phase_amounts[MAX_PHASES], converged, num_stable_phases, temp, pressure, success_marker, Y_phases[MAX_PHASES * MAX_DOF_PER_PHASE], X_phases[MAX_PHASES * MAX_COMPONENTS]
        MAX_PHASES = dynamic_sizes['MAX_PHASES']
        MAX_DOF_PER_PHASE = dynamic_sizes['MAX_DOF_PER_PHASE']
        # Updated to include ALL phase amounts AND X_phases
        results_per_condition = 7 + dynamic_sizes['MAX_COMPONENTS'] + MAX_PHASES + (MAX_PHASES * MAX_DOF_PER_PHASE) + (MAX_PHASES * dynamic_sizes['MAX_COMPONENTS'])
        ...
        try:
            ...
            if verbose:
                ...
                try:
                    ...
                    cp.cuda.runtime.deviceSynchronize()
                    print("[GPU] DEBUG: Memory transfer test completed")
                except Exception as e:
                    print(f"[GPU] DEBUG: Memory transfer test failed: {e}")
